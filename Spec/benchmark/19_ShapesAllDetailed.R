...
Ncol <- vapply(ShapesAll.Results$AMOC, length, 1L)
AMOC <- matrix(0, nrow = length(ShapesAll.Results$AMOC), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
AMOC[IJ] <- unlist(ShapesAll.Results$AMOC, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$BinSeg, length, 1L)
BinSeg <- matrix(0, nrow = length(ShapesAll.Results$BinSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
BinSeg[IJ] <- unlist(ShapesAll.Results$BinSeg, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$SegNeigh, length, 1L)
SegNeigh <- matrix(0, nrow = length(ShapesAll.Results$SegNeigh), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
SegNeigh[IJ] <- unlist(ShapesAll.Results$SegNeigh, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$PELT, length, 1L)
PELT <- matrix(0, nrow = length(ShapesAll.Results$PELT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
PELT[IJ] <- unlist(ShapesAll.Results$PELT, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$EDivisive, length, 1L)
EDivisive <- matrix(0, nrow = length(ShapesAll.Results$EDivisive), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDivisive[IJ] <- unlist(ShapesAll.Results$EDivisive, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$EDM, length, 1L)
EDM <- matrix(0, nrow = length(ShapesAll.Results$EDM), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDM[IJ] <- unlist(ShapesAll.Results$EDM, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$MFT, length, 1L)
MFT <- matrix(0, nrow = length(ShapesAll.Results$MFT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
MFT[IJ] <- unlist(ShapesAll.Results$MFT, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$Breakfast, length, 1L)
Breakfast <- matrix(0, nrow = length(ShapesAll.Results$Breakfast), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Breakfast[IJ] <- unlist(ShapesAll.Results$Breakfast, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$gSeg, length, 1L)
gSeg <- matrix(0, nrow = length(ShapesAll.Results$gSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
gSeg[IJ] <- unlist(ShapesAll.Results$gSeg, use.names = FALSE)
...
Ncol <- vapply(ShapesAll.Results$SpecDetec, length, 1L)
Spec <- matrix(0, nrow = length(ShapesAll.Results$SpecDetec), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Spec[IJ] <- unlist(ShapesAll.Results$SpecDetec, use.names = FALSE)
