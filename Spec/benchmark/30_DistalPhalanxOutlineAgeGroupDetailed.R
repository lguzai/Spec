...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$AMOC, length, 1L)
AMOC <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$AMOC), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
AMOC[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$AMOC, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$BinSeg, length, 1L)
BinSeg <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$BinSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
BinSeg[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$BinSeg, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$SegNeigh, length, 1L)
SegNeigh <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$SegNeigh), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
SegNeigh[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$SegNeigh, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$PELT, length, 1L)
PELT <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$PELT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
PELT[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$PELT, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$EDivisive, length, 1L)
EDivisive <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$EDivisive), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDivisive[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$EDivisive, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$EDM, length, 1L)
EDM <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$EDM), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDM[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$EDM, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$MFT, length, 1L)
MFT <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$MFT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
MFT[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$MFT, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$Breakfast, length, 1L)
Breakfast <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$Breakfast), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Breakfast[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$Breakfast, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$gSeg, length, 1L)
gSeg <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$gSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
gSeg[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$gSeg, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$SpecDetec, length, 1L)
Spec <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$SpecDetec), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Spec[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$SpecDetec, use.names = FALSE)
