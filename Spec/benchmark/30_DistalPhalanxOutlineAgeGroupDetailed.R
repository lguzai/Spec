...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$AMOC_10, length, 1L)
AMOC <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$AMOC_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
AMOC[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$AMOC_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$BinSeg_10, length, 1L)
BinSeg <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$BinSeg_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
BinSeg[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$BinSeg_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$SegNeigh_10, length, 1L)
SegNeigh <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$SegNeigh_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
SegNeigh[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$SegNeigh_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$PELT_10, length, 1L)
PELT <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$PELT_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
PELT[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$PELT_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$EDivisive_10, length, 1L)
EDivisive <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$EDivisive_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDivisive[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$EDivisive_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$EDM_10, length, 1L)
EDM <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$EDM_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDM[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$EDM_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$MFT_10, length, 1L)
MFT <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$MFT_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
MFT[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$MFT_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$Breakfast_10, length, 1L)
Breakfast <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$Breakfast_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Breakfast[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$Breakfast_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$gSeg_10, length, 1L)
gSeg <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$gSeg_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
gSeg[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$gSeg_10, use.names = FALSE)
...
Ncol <- vapply(DistalPhalanxOutlineAgeGroup.Results$SpecDetec_10, length, 1L)
Spec <- matrix(0, nrow = length(DistalPhalanxOutlineAgeGroup.Results$SpecDetec_10), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Spec[IJ] <- unlist(DistalPhalanxOutlineAgeGroup.Results$SpecDetec_10, use.names = FALSE)
