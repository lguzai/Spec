filename <- ".//benchmark_results//RefrigerationDevices.xlsx"
wb <- createWorkbook(type="xlsx")


Ncol <- vapply(RefrigerationDevices.Results$AMOC, length, 1L)
AMOC <- matrix(0, nrow = length(RefrigerationDevices.Results$AMOC), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
AMOC[IJ] <- unlist(RefrigerationDevices.Results$AMOC, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$BinSeg, length, 1L)
BinSeg <- matrix(0, nrow = length(RefrigerationDevices.Results$BinSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
BinSeg[IJ] <- unlist(RefrigerationDevices.Results$BinSeg, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$SegNeigh, length, 1L)
SegNeigh <- matrix(0, nrow = length(RefrigerationDevices.Results$SegNeigh), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
SegNeigh[IJ] <- unlist(RefrigerationDevices.Results$SegNeigh, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$PELT, length, 1L)
PELT <- matrix(0, nrow = length(RefrigerationDevices.Results$PELT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
PELT[IJ] <- unlist(RefrigerationDevices.Results$PELT, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$EDivisive, length, 1L)
EDivisive <- matrix(0, nrow = length(RefrigerationDevices.Results$EDivisive), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDivisive[IJ] <- unlist(RefrigerationDevices.Results$EDivisive, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$EDM, length, 1L)
EDM <- matrix(0, nrow = length(RefrigerationDevices.Results$EDM), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDM[IJ] <- unlist(RefrigerationDevices.Results$EDM, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$MFT, length, 1L)
MFT <- matrix(0, nrow = length(RefrigerationDevices.Results$MFT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
MFT[IJ] <- unlist(RefrigerationDevices.Results$MFT, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$Breakfast, length, 1L)
Breakfast <- matrix(0, nrow = length(RefrigerationDevices.Results$Breakfast), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Breakfast[IJ] <- unlist(RefrigerationDevices.Results$Breakfast, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$gSeg, length, 1L)
gSeg <- matrix(0, nrow = length(RefrigerationDevices.Results$gSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
gSeg[IJ] <- unlist(RefrigerationDevices.Results$gSeg, use.names = FALSE)
...
Ncol <- vapply(RefrigerationDevices.Results$SpecDetec, length, 1L)
Spec <- matrix(0, nrow = length(RefrigerationDevices.Results$SpecDetec), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Spec[IJ] <- unlist(RefrigerationDevices.Results$SpecDetec, use.names = FALSE)
