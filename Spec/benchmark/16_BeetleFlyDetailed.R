filename <- ".//benchmark_results//BeetleFly.xlsx"
wb <- createWorkbook(type="xlsx")


Ncol <- vapply(BeetleFly.Results$AMOC, length, 1L)
AMOC <- matrix(0, nrow = length(BeetleFly.Results$AMOC), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
AMOC[IJ] <- unlist(BeetleFly.Results$AMOC, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$BinSeg, length, 1L)
BinSeg <- matrix(0, nrow = length(BeetleFly.Results$BinSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
BinSeg[IJ] <- unlist(BeetleFly.Results$BinSeg, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$SegNeigh, length, 1L)
SegNeigh <- matrix(0, nrow = length(BeetleFly.Results$SegNeigh), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
SegNeigh[IJ] <- unlist(BeetleFly.Results$SegNeigh, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$PELT, length, 1L)
PELT <- matrix(0, nrow = length(BeetleFly.Results$PELT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
PELT[IJ] <- unlist(BeetleFly.Results$PELT, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$EDivisive, length, 1L)
EDivisive <- matrix(0, nrow = length(BeetleFly.Results$EDivisive), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDivisive[IJ] <- unlist(BeetleFly.Results$EDivisive, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$EDM, length, 1L)
EDM <- matrix(0, nrow = length(BeetleFly.Results$EDM), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
EDM[IJ] <- unlist(BeetleFly.Results$EDM, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$MFT, length, 1L)
MFT <- matrix(0, nrow = length(BeetleFly.Results$MFT), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
MFT[IJ] <- unlist(BeetleFly.Results$MFT, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$Breakfast, length, 1L)
Breakfast <- matrix(0, nrow = length(BeetleFly.Results$Breakfast), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Breakfast[IJ] <- unlist(BeetleFly.Results$Breakfast, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$gSeg, length, 1L)
gSeg <- matrix(0, nrow = length(BeetleFly.Results$gSeg), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
gSeg[IJ] <- unlist(BeetleFly.Results$gSeg, use.names = FALSE)
...
Ncol <- vapply(BeetleFly.Results$SpecDetec, length, 1L)
Spec <- matrix(0, nrow = length(BeetleFly.Results$SpecDetec), ncol = max(Ncol))
IJ <- cbind(rep(seq_along(Ncol), times = Ncol), sequence(Ncol)) 
Spec[IJ] <- unlist(BeetleFly.Results$SpecDetec, use.names = FALSE)
